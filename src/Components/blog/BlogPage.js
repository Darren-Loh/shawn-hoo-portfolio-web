...
function BlogPage() {
  ...
  let blogPosts = blogRecords && blogRecords.slice(0).reverse().map((post,index) => <BlogPost key={index} blogId={post.id} instanceID = {index} imageUrl = {post.imageUrl} recordHeader = {post.header} bodyPara = {post.bodyPara} recordDate = {post.date} recordTags = {post.tags} />)
