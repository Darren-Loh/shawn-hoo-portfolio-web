...
function BlogTagAdmin({instanceID, recordTags}) {
  ...
  useEffect(() => {
    ...
    for(let i = 0 ; i < tagContainer[instanceID].childNodes.length; i ++){
      if (i%2===0){
        //if even
        tagContainer[instanceID].childNodes[i].classList.add("darkTagAdmin");
      }
      else{
        //if odd
        tagContainer[instanceID].childNodes[i].classList.add("lightTagAdmin");
      }
    }
  });
